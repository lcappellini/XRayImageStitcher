...
cv::Mat ImageStitcher::dicom_to_cv_8(std::string path) {
    cv::Mat img16 = dicom_to_cv_16(path);
...
cv::Mat ImageStitcher::load_image(const ImageData& imgData) {
    ...
    case ImageData::Type::CV_MAT:
        out_img_res = imgData.matrix;
        break;
    }
    return out_img_res;
}

bool ImageStitcher::stitch(std::string outFilename, DetectorAlgorithm detectorAlgorithm, MatchingAlgorithm matchingAlgorithm, HomographyMethod homographyMethod)
{
    //load first image
    ...
    for (int i = 1; i < sources.size(); i++) {
        ...
        case DetectorAlgorithm::BRISK:
            //detector = cv::BRISK::create(6, 3, 1.2f); //OPTIMAL
            detector = cv::BRISK::create(5, 3, 1.2f); //OPTIMAL
            bfMatcherMethod = cv::NormTypes::NORM_HAMMING;
            descriptorType = DescriptorType::BINARY;
            break;
        ...
        case MatchingAlgorithm::SUPERGLUE:
            matcher = SuperGlue::create(superglue_model_path, 0.002f);
            matcher.dynamicCast<SuperGlue>()->init(
                kp1, kp2,
                img2.cols, rect_height,
                0, img2.rows - rect_height);
            k = 1;
            break;
        }                    
        ...
        if (k == 2) {
            ...
            float ratio_thresh = 0.6f; //WORKING
        ...
        cv::Mat mask;
        cv::Mat H = findHomography(pts2, pts1, (int)homographyMethod, 10.0, mask);
        if (H.empty()) {
        ...
        //H and overlap are found, so now i can operate with 16bit images to get the result
        ...
        //save result image with alpha blending
        apply_alpha_blending(img1_res, img2_res, gray_out, corners_transformed, overlap, H);
        if (use_raw) {
            apply_alpha_blending(img1_raw, img2_raw, gray_out_raw, corners_transformed, overlap, H);
        }
...
bool ImageStitcher::apply_alpha_blending(const cv::Mat& img1_res, const cv::Mat& img2_res, cv::Mat& gray_out, std::vector<cv::Point2f> & corners_transformed, int overlap, cv::Mat& H) {
    ...
    cv::Mat alpha_out = alpha2f + alpha1f.mul(1.0f - alpha2f);
